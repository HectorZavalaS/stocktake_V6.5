...
namespace stocktake_V4
{
    public partial class Form1 : MetroForm
    {
        ...
        siixsem_wip_control_dbEntities m_db_wip;
        ...
        public Form1()
        {
            ...
            m_db_wip = new siixsem_wip_control_dbEntities();
        ...
        {
            int result = 0;
            if(MessageBox.Show("Deseas borrar la etiqueta escaneada?", "Warning...", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
                if (!String.IsNullOrEmpty(txtSerial.Text)) {
                    result = m_db_st.deleteLabel(txtSerial.Text.Replace("'", "-").Replace("]", "|"), m_id_User).First().RESULT;
                    if (result == 1)
                    {
                        m_alerts.playSuccess();
                        MessageBox.Show("Se borro la etiqueta.");
                    }
                    else
                        if (result == -1)
                    {
                    ...
                    if (m_model != null)
                    {
                        if (m_model.BatchId != "NG")
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    GetBoxSemifinish(m_model.PartNumber, ref semifinish, m_model.Route);
                                }
                                else
                                {
                                    if (type_cont == 1 && m_user_name.Equals("box.final"))
                            ...
                            m_console.AppendText("[" + DateTime.Now.ToString() + "] " + semifinish + "\n");
        ...
        {
            if (!txtSemi.Text.Equals("PENDING"))
                printLabel();
            else
            {
                m_alerts.playWarning();
                m_dlgMessage.Message = "La etiqueta no se puede imprimir. Favor de mandar serial a IT-SEM@siix-global.com";
                m_dlgMessage.ShowDialog();
            }
        }
        ...
        private void printLabel()
        {
            try
            {
                ...
                {
                    if (!String.IsNullOrEmpty(txtScanedSerial.Text))
                    {
                        ...
                        if (zone != "NOT_FOUND")
                        {
                            ...
                            String template = m_db_st.getLabelSTV2(model, djGroup, semifinish, cgsRoute, qty, qr, mag, printer,type_cont,m_user_name,m_area,m_quarantine == true ? 1 : 0).First().label;

                            if (template != "DOESNT_EXIST")
                            {
                                if (m_db_st.insertLabel(idU, serial, model, djGroup, semifinish, cgsRoute, qty, qr, mag,type_cont, m_quarantine == true ? 1 : 0, m_id_area).First().RESULT != -1)
                                {
                    ...
                    else
                    {
                        ...
                        //MessageBox.Show("No has escaneado un serial.", "Print Error...", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        CleanFields();
                    }
                }
                else
        ...
        void GetBoxSemifinish(String model_name, ref String semifinish, String route)
        {
            try
            {
                String operation = txtSemi.Text;
                ...
                getSemiManuByModel_Result Manu = m_db_wip_cfg.getSemiManuByModel(model_name).First();

                if (Manu.SEMIFINISH.Equals("MODEL_NOT_FOUND"))
                {
                    GetSemifinish(model_name, ref semifinish, route);
                    //semifinish = txtSemi.Text = "PENDING";
                }
                else semifinish = txtSemi.Text = Manu.SEMIFINISH;
            }
            catch (Exception ex)
            {
                addLine("[" + DateTime.Now.ToString() + "] " + ex.Message + " --- " + ex.InnerException);
        ...
        void GetFinalBoxSemifinish(String model_name, ref String semifinish)
        {
            try
            {
                String operation = txtSemi.Text;
                ...
                String se_serial_read = txtScanedSerial.Text;
                //String semifinish = "";
                getSemiFGByModel_Result FG = new getSemiFGByModel_Result();
                FG = m_db_wip_cfg.getSemiFGByModel(model_name).First();

                if (FG.SEMIFINISH.Equals("MODEL_NOT_FOUND")) semifinish = txtSemi.Text = "PENDING";
                else semifinish = txtSemi.Text = FG.SEMIFINISH;
